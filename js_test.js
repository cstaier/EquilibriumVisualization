...
var fulcrum = {
    ...
    calcPosition: function() {
        if ( this.k < 0 ) {
            this.k = 0;
        } else if ( this.k <= 1 ) { // [0, 1]
            this.x = max_left + (centerX - max_left) * this.k;
        } else if ( fulcrum.k <= 11 ) { // (1, 10]
            this.x = centerX + (max_right - centerX) * (((this.k) - 1) / 10);
        } else { // (10, inf)
            this.x = max_right;
        }
...
function showDetails() {
    ...
    ctx.strokeText("fulcrum.k: " + fulcrum.k, fulcrum.x, fulcrum.y - 30);
    //ctx.strokeText("seesaw.angle: " + seesaw.angle, centerX, centerY - 30);
}
...
function draw() {
    ...
    */
    ...
    // Does not draw reactants or products if at zero percent.
